...
type Plan struct {
	...
	//Extensions requires a client API version >=2.16.
	Extensions []Extension `json:"extensions,omitempty"`
}

// Extension requires a client API version >=2.16.
//
// Extension is a set of optional JSONSchemas that describe
// extentsion that the plan support
type Extension struct {
	// ID is a globally unique ID that identifies the extension.
	ID string `json:"id"`
	// Name is the extension's display name.
	Name string `json:"name,omitempty"`
	// a relative path on the Service Broker to trigger the extension's endpoint
	Path string `json:"path"`
	// Description is a brief description of the extension, suitable for printing by
	// a CLI.
	Description string `json:"description"`
